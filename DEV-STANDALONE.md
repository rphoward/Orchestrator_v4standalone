...
**Stage tracking modes:** the backend reads **`stage_tracking_mode`** from SQLite **`config`** with values **`off`**, **`hybrid`**, or **`semantic`**. Missing or invalid values fall back to **`hybrid`**. **`stage_tracking_judge_interval`** defaults to **4**. The API is **`GET` / `PUT /api/config/stage-tracking`** with JSON keys **`stage_tracking_mode`** and **`stage_tracking_judge_interval`**; it also accepts and returns the plan aliases **`mode`** and **`judge_interval_turns`**.

**Stage completion decision:** **`InterviewStageCompletionJudge`** (a core port; Gemini adapter lives in [`infrastructure/ai/gemini_stage_completion_judge.py`](infrastructure/ai/gemini_stage_completion_judge.py)) is the semantic judge. The judge reads a per-stage completion prompt from [`runtime/prompts/stage_completion/`](runtime/prompts/stage_completion/) and returns a structured verdict (`stage_complete`, `confidence`, `reason`, `evidence_found`, `missing_topics`). The verdict flips the stage flag only when `confidence >= 0.75` and `stage_complete is True`. When no `GEMINI_API_KEY` is configured, **`bootstrap.rebind_llm_gateway()`** installs `FakeStageCompletionJudge(default_reason="judge_error: offline stub")`, which keeps offline dev, `pytest`, and `python run_dev.py --smoke` on the existing fallback path after a judge attempt.

**Hybrid progress:** hybrid mode keeps compact per-session counters in **`sessions.stage_progress`** JSON. Each eligible user turn for agents **1..4** updates only that handled stage's counters. The compact counters can mark a stage as a **candidate** for semantic review, but they do not flip stage flags. Hybrid skips the judge until the stage has at least two user messages, meaningful evidence exists, and the current user message is not a very short test-like input. It runs the semantic judge when the compact progress is candidate complete, the user explicitly asks to advance/change stage, final report/export is requested, or the interval expires. Manual routing still updates progress for agents **1..4** because manual routing only means the consultant chose the agent. Agent **5** is skipped by progress, judge, and stage flags.

**Semantic/off modes:** semantic mode keeps the previous eligible-turn behavior: agents **1..4** call the judge on each turn. Off mode does no automatic stage tracking. Manual turns to agent 5 (Grand Synthesis) never flip any stage flag. A domain-authoring meta-prompt lives at [`runtime/prompts/Domain Library/stage_completion_prompt_author.md`](runtime/prompts/Domain Library/stage_completion_prompt_author.md) and is used offline to write the per-stage completion prompt bodies from each stage's interview prompt.

**Turn observability (read-only, no new LLM calls):** each auto or manual chat turn appends one JSON object to **`sessions.stage_tracking_log`** (capped array, latest entries kept). The object includes **`turn_endpoint`** (`auto` or `manual`), **`stage_tracking_mode`**, gate **`gate_reason`** strings (same values as the judge gate, e.g. `fewer_than_two_user_messages`, `semantic_mode`, `no_meaningful_evidence`, `short_or_test_like`, `ineligible_agent`), and optional judge **`verdict`**. The turn **POST** responses also echo a **`stage_tracking`** object. **GET** `/api/sessions/<id>/stage-tracking` returns `{"entries": [ ... ]}` from SQLite only — it does not run `RefreshStageTrackingBeforeReport` or any judge.
...
**What the file holds:** sessions, conversations, routing logs, agents (including per-agent model ids), prompt templates, **`config`** (model registry JSON, router model id, stage tracking settings, and other keys). Paths default under **`runtime/`** unless **`ORCHESTRATOR_DB_PATH`** points elsewhere.
